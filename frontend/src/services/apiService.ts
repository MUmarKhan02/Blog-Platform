...
class ApiService {
  ...
  // Register a new user
public async register(request: RegisterRequest): Promise<{ id: string; name: string; email: string }> {
  const response: AxiosResponse<{ id: string; name: string; email: string }> = await this.api.post('/users/register', request);
  ...
  public async getUserProfile(): Promise<{ id: string; name: string; email: string, role: 'USER' | 'ADMIN' }> {
    const response: AxiosResponse<{ id: string; name: string; email: string, role: 'USER' | 'ADMIN' }> = await this.api.get('/users/me');
    return response.data;
  }
